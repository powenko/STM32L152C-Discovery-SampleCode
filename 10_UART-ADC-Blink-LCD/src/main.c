...
{
	   GPIO_InitTypeDef g;
	  g.GPIO_Pin = GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_15  ;    // ?? 6,7 ?
    g.GPIO_Mode = GPIO_Mode_OUT;           // ???????
    //g.GPIO_PuPd = GPIO_PuPd_UP;            // ??????
    //g.GPIO_PuPd = GPIO_PuPd_DOWN;      // ??????
    g.GPIO_Speed = GPIO_Speed_40MHz;  // ?? GPIO ???40 MHz

    RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ENABLE);    // ?? GPIOB

    GPIO_Init(GPIOA, &g);    // ??? GPIO B
	
	  g.GPIO_Pin = GPIO_Pin_2  ;    // ?? 6,7 ?
    g.GPIO_Mode = GPIO_Mode_OUT;           // ???????
    //g.GPIO_PuPd = GPIO_PuPd_UP;            // ??????
    //g.GPIO_PuPd = GPIO_PuPd_DOWN;      // ??????
    g.GPIO_Speed = GPIO_Speed_40MHz;  // ?? GPIO ???40 MHz

    RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOD, ENABLE);    // ?? GPIOB

    GPIO_Init(GPIOD, &g);    // ??? GPIO B
}
...
void NVIC_Configuration(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = TIM6_IRQn ;				 //????2
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;	     //????? 0
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;			 //????2  
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;				 //??
  NVIC_Init(&NVIC_InitStructure);

	  NVIC_InitStructure.NVIC_IRQChannel = TIM7_IRQn ;				 //????2
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;	     //????? 0
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;			 //????2  
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;				 //??
  NVIC_Init(&NVIC_InitStructure);
...
int duty_cycle=1;
int count=0;
int LED[]={1,100,200,400,300,200,100};
		...
		*/
			for(i=0;i<4;i++)
			{
				if(LED[i]<count)
					...
					{
						case 0:
							GPIO_WriteBit(GPIOA,GPIO_Pin_11,status);
						  break;
						case 1:
							GPIO_WriteBit(GPIOA,GPIO_Pin_12,status);
						  break;
						case 2:
							GPIO_WriteBit(GPIOA,GPIO_Pin_15,status);
						  break;
						case 3:
							GPIO_WriteBit(GPIOD,GPIO_Pin_2,status);
						  break;
						case 4:
							GPIO_WriteBit(GPIOC,GPIO_Pin_10,status);
						  break;
						case 5:
							GPIO_WriteBit(GPIOC,GPIO_Pin_11,status);
						  break;
						case 6:
							GPIO_WriteBit(GPIOC,GPIO_Pin_12,status);
						  break;
						}
